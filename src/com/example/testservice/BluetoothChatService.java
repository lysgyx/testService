...
public class BluetoothChatService {
	...
	public static final int STATE_LISTEN = 1; // 正在侦听传入的连接
	public static final int STATE_CONNECTING = 2; // 正在连接设备
	public static final int STATE_CONNECTED = 3; // 已经连接到远程设备
	...
	private synchronized void setState(int state) {
		...
		mState = state;
		// 给新状态的处理程序，界面活性可以更新
		bHandler.obtainMessage(CommunicationService.MESSAGE_STATE_CHANGE, state, -1).sendToTarget();
	}
	...
	public synchronized void connected(BluetoothSocket socket, BluetoothDevice device) {
		...
		Message msg = bHandler.obtainMessage(CommunicationService.MESSAGE_DEVICE_NAME);

		Log.i(TAG, "MESSAGE_DEVICE_NAME:" + device.getName());
	...
	private void connectionFailed() {
		setState(STATE_LISTEN);
		// 发送失败的信息带回活动
		Message msg = bHandler.obtainMessage(CommunicationService.MESSAGE_TOAST);
		Bundle bundle = new Bundle();
	...
	private void connectionLost() {
		...
		Message msg = bHandler.obtainMessage(CommunicationService.MESSAGE_TOAST);
		Bundle bundle = new Bundle();
		bundle.putString(TOAST, "装置连接丢失");
		msg.setData(bundle);
		bHandler.sendMessage(msg);
	}

	/**
	 * 侦听传入的连接。它的行为 就像一个服务器端的客户端。它运行直到连接被接受 （或取消）。
	 */
	...
	private class ConnectedThread extends Thread {
		...
		public void run() {
			...
			while (true) {
				try {
					...
					// 发送获得的字节的用户界面
					bHandler.obtainMessage(CommunicationService.MESSAGE_READ, bytes, -1, buffer).sendToTarget();
					// 为修改标志位提供等待时间
					try {
						ConnectedThread.sleep(100);
					} catch (InterruptedException e) {
		...
		public void write(byte[] buffer) {
			try {
				...
				// 分享发送的信息到Activity
				bHandler.obtainMessage(CommunicationService.MESSAGE_WRITE, -1, -1, buffer).sendToTarget();
			} catch (IOException e) {
